...
class JobController extends Controller
{
    ...
    public function charges_store($request,$id)
    {
        ...
        $cr_bill_invoice = $request->cr_bill_invoice;
        foreach($cr_bill_invoice as $key => $value) {
            $job_receivable = new JobReceivable();
            $job_receivable->job_id = $job->id;
            ...
            $job_receivable->crp_shared = $request->crp_shared[$key];
            $job_receivable->crp_code = $request->crp_code[$key];
            $job_receivable->crp_name_1 = $request->crp_name_1[$key];
            $job_receivable->crp_manual = $request->crp_manual[$key]; 
            ...
            $job_receivable->crp_tax_apple = $request->crp_tax_apple[$key];
            ...
            $job_receivable->crp_code_2 = $request->crp_code_2[$key];
            $job_receivable->crp_name_2 = $request->crp_name_2[$key];
    ...
    public function get_data(Request $request)
    {
        ...
        if($type == "first") {
            $data = Job::orderBy('id', 'asc')->first();
        }
        else if($type == "last") {
            $data = Job::orderBy('id', 'desc')->first();
        }
        else if($type == "forward") {
            $data = Job::where('id', '>', $id)->first();
        }
        else if($type == "backward") {
            $data = Job::where('id', '<', $id)->orderBy('id', 'desc')->first();
        }
    ...
    public function get_quot(Request $request)
    {
        ...
        if(isset($request->quot_id) && $request->ajax()){
            ...
            $arr["quotation_detail"] = QuotationDetail::where('quotation_id', $request->quot_id)->first();
            $arr["routing"] = QuotationRouting::where('quotation_id', $request->quot_id)->first();
