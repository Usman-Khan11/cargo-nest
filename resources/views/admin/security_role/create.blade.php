...
@section('top_nav_panel')
    <div class="col-md-4">
        <div class="d-flex">
            <div class="plus" onclick="roleFormReset('/admin/security-role/store')">
...
@section('panel')
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="row">
                    ...
                    <div class="card-header">
                        <div class="row">
                            ...
                            <div class="col-5 text-end">
                                <button type="button" onclick="permissionUpdate()" class="btn btn-primary">Update</button>
                            </div>
                        </div>
                    </div>
                    <table class="datatables-basic table">
                        ...
                        <tbody class="detail_repeater">
                            <tr>
                                ...
                                <td>
                                    <input class="form-control keys" type="text" name="keys[]" readonly
                                        style="width: 100%" />
                                </td>
                            </tr>
                        </tbody>
            ...
            <div class="col-md-4">
                <form id="myForm" method="post" action="{{ route('admin.security_role.store') }}"
...
@push('script')
    ...
    <script>
        ...
        function edit_row(e, data) {
            ...
            $('.quotation_record tr').removeClass("row_active");
            if (data) {
                $(".name").val(data.name);
                $("#myForm").attr("action", "{{ route('admin.security_role.update') }}");
                $("input[name=id]").val(data.id);

                $(e).addClass("row_active");
                $(".jstree-clicked").parent().click();
            }
        }
        ...
        function getKeys(nav_id) {
            ...
            $.get("/admin/security-role/create", {
                _token: '{{ csrf_token() }}',
                nav_id,
                role_id
            }, function(res) {
        ...
        function permissionUpdate() {
            ...
            $(perm).each(function(i, v) {
                let key_id = $(v).parent().find("input.key_id").val();
                let nav_id = $(v).parent().find("input.nav_id").val();
                permArr.push({
                    key_id: key_id,
                    nav_id: nav_id,
                    role_id: role_id
            ...
            $.post("/admin/security-role/update", {
                _token: '{{ csrf_token() }}',
                permArr,
                role_id,
                name: 'perm'
